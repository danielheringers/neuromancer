...
- workflow cross-platform `alicia-ci` com jobs:
  ...
  - `e2e_flow`
2. `ALICIA-MVP-019`:
- testes E2E do fluxo principal (caso feliz e caso negacao/expiracao) no `codex-alicia-ui`.
3. `ALICIA-MVP-020`:
...
4. `cargo fmt -- --config imports_granularity=Item`
5. `cargo clippy --fix --all-features --tests --allow-dirty -p codex-alicia-ui`
