...
4. Validacao de adapters com binarios reais (status atual):
- `codex` presente no host e respondendo `codex-cli 0.101.0` em `--version`.
- provider Claude validado com binario `claude` (`claude --version` -> `2.1.45 (Claude Code)`).
- smoke real do adapter `claude-code` aprovado no host atual.
...
2. Base: `main`
3. Head: `neuromancer`
4. Commit da rodada atual: `e369a74f71bd25b6adad5a9efee1325de9e41e18`
...
2. `claude` disponivel no host e validado com:
...
- run_number: `14`
- status no momento do registro: `queued` (fila de runners do repositório).
...
- `claude --version` no host atual retorna `2.1.45 (Claude Code)`.
- `cargo test -p codex-alicia-adapters real_provider_claude_code_smoke -- --exact --nocapture` passou com `ALICIA_REAL_PROVIDER_CLAUDE_CODE=1` e `ALICIA_CLAUDE_CODE_BIN=claude`.
- risco residual de validacao real do provider fechado para este host (2026-02-18).

## Procedimento objetivo de validacao real do provider `claude-code`
1. Executar em host de release com binario disponivel no PATH:
```powershell
claude --version
cd codex-rs
...
```
2. Observacao operacional:
- o smoke autodetecta `claude` e `claude-code` se `ALICIA_CLAUDE_CODE_BIN` nao estiver definido; usar essa env var apenas para forcar um executavel especifico.
3. Evidencia de aceite esperada:
- `test real_provider_claude_code_smoke ... ok`
- comando `claude --version` retornando versao sem erro.
4. Origem do smoke test:
- `codex-rs/alicia-adapters/tests/real_provider_smoke.rs`
5. Apos executar no host alvo, atualizar este documento com o transcript resumido e remover o status de risco residual.

## Revalidacao apos fix de intermitencia (2026-02-18)
1. Commit com fix: `e369a74f71bd25b6adad5a9efee1325de9e41e18`
2. Mudanca: estabilizacao do teste `start_pipe_session_emits_started_output_and_finished_events` em `codex-rs/alicia-core/src/session.rs`.
3. Workflow run (`pull_request`): `https://github.com/danielheringers/neuromancer/actions/runs/22140345314`
4. Run number: `19`
5. Resultado final: `success` (9/9 jobs verdes)
6. Jobs por matriz:
- `Alicia E2E Flow - macos-latest`: `https://github.com/danielheringers/neuromancer/actions/runs/22140345314/job/64002696988`
- `Alicia E2E Flow - ubuntu-latest`: `https://github.com/danielheringers/neuromancer/actions/runs/22140345314/job/64002696992`
- `Alicia E2E Flow - windows-latest`: `https://github.com/danielheringers/neuromancer/actions/runs/22140345314/job/64002696924`
- `Alicia Policy and Approval Scenarios - macos-latest`: `https://github.com/danielheringers/neuromancer/actions/runs/22140345314/job/64002697036`
- `Alicia Policy and Approval Scenarios - ubuntu-latest`: `https://github.com/danielheringers/neuromancer/actions/runs/22140345314/job/64002697168`
- `Alicia Policy and Approval Scenarios - windows-latest`: `https://github.com/danielheringers/neuromancer/actions/runs/22140345314/job/64002696899`
- `Alicia Suite Minima - macos-latest`: `https://github.com/danielheringers/neuromancer/actions/runs/22140345314/job/64002697132`
- `Alicia Suite Minima - ubuntu-latest`: `https://github.com/danielheringers/neuromancer/actions/runs/22140345314/job/64002697016`
- `Alicia Suite Minima - windows-latest`: `https://github.com/danielheringers/neuromancer/actions/runs/22140345314/job/64002696947`

## Revalidacao apos hardening do smoke real Claude (2026-02-18)
1. Commit: `3c38be3de1de7795ed381cfbc56f653fca2b70ba`
2. Mudanca: autodetect de binario `claude`/`claude-code` no smoke real do provider (`codex-rs/alicia-adapters/tests/real_provider_smoke.rs`).
3. Workflow run (`pull_request`): `https://github.com/danielheringers/neuromancer/actions/runs/22144213751`
4. Run number: `32`
5. Resultado final: `success` (9/9 jobs verdes)
6. Jobs por matriz:
...
## Notas de release da candidata
1. Documento consolidado: `Alicia/15-notas-release-candidata-mvp-018-020.md`.
